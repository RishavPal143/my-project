...
// Finds products with a specific variant color and only returns details of that matching variant
router.get('/variants/:color', async (req, res) => {
    ...
    try {
        const productsWithSpecificVariant = await Product.aggregate([
            {
                // Stage 1: Filter documents that contain the required nested color
                $match: { 'variants.color': color }
            },
            {
                // Stage 2: Project specific fields and use $filter to isolate the matching nested documents
                $project: {
                    _id: 0,
                    product_name: '$name',
                    product_price: '$price',
                    // Use $filter to select only the variant objects where the color matches the input
                    matching_variants: {
                        $filter: {
                            input: '$variants',
                            as: 'variant',
